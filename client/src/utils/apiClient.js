/**
 * API client with consistent error handling and retry logic
 */

import { retryWithBackoff } from './networkResilience';
 ...
 * @param {boolean} options.retry - Enable retry with exponential backoff (default: false)
...
export const apiRequest = async (endpoint, options = {}) => {
  const {
    ...
    onProgress,
    retry = false, // Enable retry for idempotent requests
    retryOptions = {},
  } = options;
  ...
  if (retry) {
    return retryWithBackoff(executeRequest, {
      ...retryOptions,
      onRetry: (info) => {
        console.log(`Retrying request to ${endpoint} (attempt ${info.attempt}/${info.maxRetries})`);
